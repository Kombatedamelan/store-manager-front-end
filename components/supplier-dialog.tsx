...
export function SuppliersDialog({ open, onOpenChange, supplier, onSave }: any) {
      ...
      <DialogContent className="sm:max-w-[500px] bg-amber-100 border-4 border-amber-800">
        <form onSubmit={handleSubmit}>
          <DialogHeader className="bg-amber-800 text-white p-4 -mx-6 -mt-6 rounded-t-lg border-b-2 border-amber-900">
            <DialogTitle>{supplier ? "Modifier un fournisseur" : "Ajouter une nouveau fournisseur"}</DialogTitle>
            <DialogDescription className="text-amber-100">
              {supplier ? "Mettez à jour les détails de la categorie." : "Remplissez les détails pour la nouvelle categorie."}
            </DialogDescription>
          </DialogHeader>
